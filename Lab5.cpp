...
void Draw (void)
{
   ...
   for (int i=0; i < 5; i++) {
      ...
      ModelTrans.translate(vec3(g_tx+width+(r*0.25), g_ty+(r*0.25), g_tz));
      ModelTrans.scale(r*0.5, r*0.5, 1);
      SetModel();
      /* set the color in the shader */
      glUniform3f(h_uColor, 0.6*sin(i*5*PI/180), 0.6*cos(i*10*PI/180), 0.6*sin(i*20*PI/180));
      // draw!
   ...
   //   ModelTrans.pushMatrix();
   //   /* set up where to draw the box */
   //   ModelTrans.translate(vec3(g_tx+i*1.5, g_ty+sin(i*30*PI/180)*2, g_tz+cos(i*30*PI/180)*2));
   //   SetModel();
   //   /* set the color in the shader */
   //   glUniform3f(h_uColor, 0.6*sin(i*5*PI/180), 0.6*cos(i*10*PI/180), 0.6*sin(i*20*PI/180));
   //   // draw!
   ...
   //   ModelTrans.pushMatrix();
   //   /* set up where to draw the box */
   //   ModelTrans.translate(vec3(g_tx+(6+i)*1.5, g_ty+sin(i*30*PI/180)*2, g_tz+cos(i*30*PI/180)*2));
   //   SetModel();
   //   /* set the color in the shader */
   //   glUniform3f(h_uColor, 1*sin(i*10*PI/180), 1*cos(i*20*PI/180), 1*sin(i*5*PI/180));
   //   // draw!
   //   glDrawElements(GL_TRIANGLES, g_CiboLen, GL_UNSIGNED_SHORT, 0);
   //   ModelTrans.popMatrix();
   //}
